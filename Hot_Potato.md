...
int melody[] = {  NOTE_B4,-4, NOTE_E5,-4, NOTE_B4,-4, NOTE_E5,-4, 
...
void loop() {
  ...
  if (buttonPress == HIGH){
    duration = random(10,60); //set game length (s)
     }
      if (duration*1000 >= noteTime && duration > 0){ // mid game  
    ...
    noTone(buzzerPin);
    turnChance = random (1,16);
    if (turnChance == 5 || (stayOn > 1 && stayOn < 8)){
                        digitalWrite(turnPin, HIGH); //turn on
                        stayOn = stayOn+1;
    }else{
                        digitalWrite(turnPin, LOW); //turn off
                        stayOn =1;

    }                 
    noteTime =noteTime+noteDuration;

    if (duration*1000 <= noteTime && noteTime > 0){
          digitalWrite(ledPin, HIGH); //turn on
          tone(buzzerPin,110);
          delay (3000);
          noteTime = 0;
